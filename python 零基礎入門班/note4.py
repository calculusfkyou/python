...
刪除串列元素:
串列名稱.remove(串列元素)    #remove()是刪除串列中第一個指定的串列元素，若串列元素不再串列中，這樣會發生錯誤。
ex:
list = [1,2,3,4]
list.remove(2)
print(list)   #[1, 3, 4]

串列名稱.pop([index])       #pop()由串列中取出元素，同時串列會將該元素移除。pop()可以有參數也可以沒有:若沒有參數，會取出最後一個元素;如果有，參數的資料型態必須為整數，就會取出以參數為索引值的元素。
ex:
list = [1,2,3,4,5,6]
